...
#include <iomanip>
#include <memory>
#include "nuitrack/middleware/android/NuitrackManager.h"
#include "nuitrack/middleware/android/USBGateKeeper.h"
#endif

#ifdef ANDROID

#include <android/log.h>
#define LOGGING_OPENNI(...){__android_log_print(ANDROID_LOG_INFO, "OPENNI", __VA_ARGS__);}

#else

#include <stdio.h>
#include <cstdlib>
#define LOGGING_OPENNI(...){ printf( __VA_ARGS__ ); }
...
private:
	...
	public:
		 ...
		 */
		inline XnStatus Init(const XnChar* configDir = NULL)
		{
			XnContext* pContext = NULL;
#if defined ANDROID
			tdv::nuitrack::middleware::android::NuitrackManager nuitrackMgr;

			std::string dataPath = nuitrackMgr.getDataPath() + "/";

			configDir = dataPath.c_str();
#endif

			XnStatus nRetVal = xnInit(&pContext, configDir);
			XN_IS_STATUS_OK(nRetVal);
...
#if defined ANDROID
			using namespace tdv::nuitrack::middleware::android;
			std::shared_ptr<USBGateKeeper> keeper = std::make_shared<USBGateKeeper>();

			xn::NodeInfoList possibleChains;
			EnumerateProductionTrees(XN_NODE_TYPE_DEVICE, NULL, possibleChains);

			for(xn::NodeInfoList::Iterator i = possibleChains.Begin(); i != possibleChains.End(); ++i)
			{
				xn::NodeInfo node = *i;
				std::string vendorInfo = (std::string)node.GetCreationInfo();
				std::stringstream ss;
				int deviceVID, devicePID;
				ss << vendorInfo.substr(0, 4) << " " << vendorInfo.substr(5, 4);
				ss >> std::hex >> deviceVID;
				ss >> std::hex >> devicePID;

				LOGGING_OPENNI("OpenNI found device: %x %x\n", deviceVID, devicePID);

				int status = keeper->requestPermission(deviceVID, devicePID);

				LOGGING_OPENNI("Permission access: %d\n", status);

				// wait for USB permission granted
				int fd = keeper->getFileDescriptor();
				LOGGING_OPENNI("Permission FD: %d\n", fd);
			}
#endif
...
#if defined ANDROID
		XnUInt32 outputModesNumber = GetSupportedMapOutputModesCount();
		std::vector<XnMapOutputMode> outputModes(outputModesNumber);
		GetSupportedMapOutputModes((XnMapOutputMode *)outputModes.data(), outputModesNumber);
		if (outputModesNumber > 0)
		{
			SetMapOutputMode(outputModes[0]);
		}
#endif
